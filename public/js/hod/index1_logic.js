...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function syncProblemEmptyState(content) {
        ...
        if (!emptyRow) {
            ...
            emptyRow.innerHTML = '<td colspan="4" class="py-8 text-center text-gray-500 dark:text-gray-400">No problems match the current filters.</td>';
    ...
    function addToBookmarkedTab(problemRow) {
        ...
        if (actionsCell) {
            actionsCell.innerHTML = `
                <a href="/college/hod/view-problem/${problemId}"
    ...
    async function saveBookmarkState(problemId, isBookmarked) {
        try {
            ...
            if (!id) return;
            if (isBookmarked) {
                await fetch('/college/hod/api/problem-bookmarks', {
                    method: 'POST',
            ...
            await fetch(`/college/hod/api/problem-bookmarks/${id}`, { method: 'DELETE' });
        } catch (e) {
    ...
    async function initBookmarks() {
        try {
            const res = await fetch('/college/hod/api/problem-bookmarks');
            if (!res.ok) return;
    ...
    // Contest Logic moved to contest.ejs (API Integrated)
    // --- End of Removed Redundant Contest Logic ---
...
var reportData = window.__hodReportData || (window.__hodReportData = {
    Students: {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (facultySearchInput) {
        facultySearchInput.addEventListener('input', (e) => {
            ...
            const rows = document.querySelectorAll('.divide-y tr.group');
            rows.forEach(row => {
    ...
    if (subjectFilter) {
        subjectFilter.addEventListener('change', (e) => {
            ...
            rows.forEach(row => {
                const trigger = row.querySelector('.subject-count-trigger');
                if (trigger && trigger.dataset.subjects) {
                    try {
                        const subjects = JSON.parse(trigger.dataset.subjects);
    ...
    const studentFilters = document.querySelectorAll('#student-list-container select');
    
    function applyStudentFilters() {
        const term = studentSearchInput ? studentSearchInput.value.toLowerCase() : '';
        const rows = document.querySelectorAll('#studentsTable tbody tr');
        
        // Get filter values
        const ratingFilter = studentFilters[0] ? studentFilters[0].value : 'All Tiers';
        const yearFilter = studentFilters[1] ? studentFilters[1].value : 'All Years';
        const langFilter = studentFilters[2] ? studentFilters[2].value : 'All Languages';
        const sortOrder = studentFilters[3] ? studentFilters[3].value : 'Recent';
        ...
        rows.forEach(row => {
            ...
            const matchesYear = yearFilter === 'All Years' || data.year === yearFilter;
            
            // Rating Tier (Mock logic based on points/data)
            let matchesTier = true;
            if (ratingFilter !== 'All Tiers') {
                const points = parseInt(data.points || 0);
                if (ratingFilter.includes('Grandmaster') && points <= 2400) matchesTier = false;
                if (ratingFilter.includes('Master') && (points < 2000 || points > 2400)) matchesTier = false;
                if (ratingFilter.includes('Expert') && (points < 1600 || points > 2000)) matchesTier = false;
            }

            row.style.display = (matchesSearch && matchesYear && matchesTier) ? '' : 'none';
        });

        // Basic Sorting (Optional, but user asked for search/filter work)
        if (sortOrder === 'Name') {
            const tbody = document.querySelector('#studentsTable tbody');
            const sortedRows = Array.from(rows).sort((a, b) => (a.dataset.name || '').localeCompare(b.dataset.name || ''));
            sortedRows.forEach(r => tbody.appendChild(r));
        } else if (sortOrder === 'Rank') {
            const tbody = document.querySelector('#studentsTable tbody');
            const sortedRows = Array.from(rows).sort((a, b) => parseInt(a.dataset.rank || 9999) - parseInt(b.dataset.rank || 9999));
            sortedRows.forEach(r => tbody.appendChild(r));
        }
    }
    ...
    studentFilters.forEach(f => f.addEventListener('change', applyStudentFilters));
    ...
    if (logoutBtnGlobal) {
        logoutBtnGlobal.addEventListener('click', (e) => {
            e.preventDefault();
            globalLogout();
        });
    }
